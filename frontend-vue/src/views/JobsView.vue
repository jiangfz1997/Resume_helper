<template>
  <main class="jobs-page">
    <header class="hero">
      ...
      <div class="snapshot"><i /><div><b>{{ pendingCount }} pending · {{ unreadRunCount }} unread runs</b><small>{{ unviewedCount }} roles you have not opened</small></div></div>
    </header>
    ...
    <section v-if="!loading && !loadError && visibleJobs.length" class="workspace">
      ...
      <aside v-if="selectedJob" class="detail">
        ...
        <section :class="['reasoning', { pending: !assessmentFor(selectedJob.jobId)?.coarseScoreReasoning }]">
          <span class="eyebrow">{{ assessmentFor(selectedJob.jobId)?.coarseScoreReasoning ? 'WHY THIS FITS YOU' : 'NOT SCORED FOR YOU YET' }}</span>
          <p>{{ assessmentFor(selectedJob.jobId)?.coarseScoreReasoning || 'Save a scoring profile, then the scheduled scoring run will evaluate this role for your account.' }}</p>
...
import type { DashboardJob, DashboardJobSummary, DashboardRunSummary, DashboardUserState, JobUserState, JobUserStatus, WorkplaceType } from '../jobs/models'
...
const runOptions = computed(() => [{ label: `All runs${unreadRunCount.value ? ` · ${unreadRunCount.value} unread` : ''}`, value: 'all' }, ...runs.value.map((run) => ({
  label: `${isRunViewed(run.runId) ? '' : '● '}${formatRun(run)} · ${run.newJobsCount} new`,
  value: run.runId,
}))])

const pendingCount = computed(() => jobs.value.filter((job) => job.eligibilityStatus !== 'excluded' && !['applied', 'rejected'].includes(statusFor(job.jobId))).length)
const unviewedCount = computed(() => jobs.value.filter((job) => job.eligibilityStatus !== 'excluded' && !isViewed(job.jobId) && !['applied', 'rejected'].includes(statusFor(job.jobId))).length)
...
onMounted(async () => {
  try {
    const [loadedJobs, loadedRuns, loadedState, loadedProfile] = await Promise.all([
...
function assessmentFor(jobId: string): JobUserState | undefined {
  const assessment = userState.value.jobs.find((job) => job.jobId === jobId)
