"""Build a Linux-targeted deployment zip for the jobspy Lambda.

Same dependency handling as job-discovery/lambda_probe/build.py (the
NUMPY==1.26.3 pin bypass, the tls-client platform-binary pruning) plus one
more step: job_discovery itself is copied in, since it is not published
anywhere to pip install -- see job-discovery/lambda_workday/build.py for the
same pattern on the lighter Workday Lambda.
...
the build machine's platform, so a Windows box produces a valid Lambda package.
    ...
    python build.py [--arch x86_64|arm64]

Output: dist/lambda-jobspy.zip
"""
...
DIST = HERE / "dist" / "lambda-jobspy.zip"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the jobspy Lambda zip")
